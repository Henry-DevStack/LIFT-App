...
export const db = {
  ...
  // ---- Treino em andamento ----
  // Guarda o progresso do treino atual (cargas, reps, séries marcadas) pra
  // que sair do app / ir pra tela inicial não perca nada. É apagado quando
  // o treino é finalizado ou descartado.
  getActiveSession: () => read(KEYS.ACTIVE_SESSION, null),
  ...
  // Última carga/reps registradas pra um exercício (por nome), usada pra
  // mostrar a coluna "anterior" durante o treino.
  getLastPerformance: (exerciseName) => {
    for (const log of db.getWorkoutLogs()) {
      const ex = log.exercises?.find((e) => e.name === exerciseName);
      if (ex && (ex.loadsUsed?.length || ex.repsUsed?.length)) {
        return { loads: ex.loadsUsed || [], reps: ex.repsUsed || [], date: log.date };
      }
    }
    return null;
  },
  ...
  saveChatConfig: (config) => {
    write(KEYS.CHAT_CONFIG, config);
    return config;
  },
  getChatHistory: () => read(KEYS.CHAT_HISTORY, []),
  saveChatHistory: (messages) => {
    // Guarda só as últimas 50 mensagens pra não estourar o localStorage.
    const trimmed = messages.slice(-50);
    write(KEYS.CHAT_HISTORY, trimmed);
    return trimmed;
  },
  clearChatHistory: () => {
